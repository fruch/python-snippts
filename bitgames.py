...
def DVB_MJD_to_string( bit_structure=None , byte_array=None ):
	...
	if (len(byte_array) != 5):
		...
		return "Didn't parse" 
	elif (ByteToHex(byte_array.tostring()) == "FF FF FF FF FF"):
		log.error("No a DVB MJD: 0xFFFFFFFFFF")
		return "Unlimited"
	elif (ByteToHex(byte_array.tostring()) == "00 00 00 00 00"):
		log.error("No a DVB MJD: 0x0000000000")
		return "None"
	
	log.debug( ByteToHex(byte_array.tostring()))
...
def main():
	...
	if(input_filename != None):
		...
		data = array.array('B', HexToByte(file.readline()))
