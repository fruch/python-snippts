...
logger = logging.getLogger("secheduler")
...
class Task(object):
    ...
    def run(self):
        logger.debug("Running %s task, scheduled at: %s" % (self.name, self.scheduled_time,))
        if not self.halt_flag.isSet():
            try:
                ...
                logger.debug("Scheduled next run of %s for: %s" % (self.name, self.scheduled_time,))
...
class Scheduler(Thread):
    ...
    def drop(self, task_receipt):
        ...
        try:
            ...
            logger.error('Invalid task receipt: %s' % (task_receipt,))
        self.tasks_lock.release()
    def halt(self):
        self.halt_flag.set()
    ...
    def run(self):
        while 1:
            ...
            if receipt != None:
                ...
                logger.debug("Next task is %s in %s seconds" % (self.tasks[receipt].name, time_to_wait,))
                self.halt_flag.wait(secs_to_wait)
                try:
                    try:
                        self.tasks_lock.acquire()
                        task = self.tasks[receipt]
                        logger.debug("Running %s..." % (task.name,))
                        task.run()
                    finally:
                        self.tasks_lock.release()
                except Exception, e:
...
def every_x_secs(x):
    ...
    """
    return lambda last, *arg_k: last + datetime.timedelta(seconds=x)
...
def every_x_mins(x):
    ...
    in the future from a given argument.
    """
    return lambda last, *arg_k: last + datetime.timedelta(minutes=x)    
...
def daily_at(time):
    ...
    """
    return lambda last, *arg_k: datetime.datetime.combine(last + datetime.timedelta(days=1), time)
...
class RunUntilSuccess(object):
    ...
    """
    def __init__(self, func, args=None, num_tries=10):
        self.func = func
    ...
    def __call__(self):
        ...
        while not is_success and try_count < self.num_tries:
            ...
            try:
                ...
                logger.exception(e)
                logger.error("Task failed on try #%s" % (try_count,))
                continue

        if is_success:
            logger.info("Task %s was run successfully." % (self.func.__name__,))
        else:
            logger.error("Success was not achieved!")
